    ...
    for transform in st.session_state.transformations:
        ...
        try:
            if op_type == 'filter':
                ...
                # Simple eval - in production use safer eval
                exec_context = {
                    **{col: data[col] for col in data.columns},
                    'pd': pd,
                }
                result = eval(transform['expression'], {"__builtins__": {}}, exec_context)
                ...
                data = data.groupby(transform['group_by']).agg(agg_dict).reset_index()
...
with st.sidebar:
    ...
    if uploaded_file:
        try:
            ...
            st.session_state.transformations = []
            st.session_state.r_code = ""
            st.success(f"✅ Loaded: {st.session_state.original_data.shape[0]} rows, {st.session_state.original_data.shape[1]} cols")
        except Exception as e:
...
if st.session_state.data is not None:
    ...
    with col1:
        st.text_area("R Code", value=r_code, height=200, disabled=True)
    with col2:
        st.download_button(
            label="📥 Download .R",
            data=r_code,
